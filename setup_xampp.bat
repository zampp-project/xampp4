...
>> "install\_xampp_path_fix.ps1" echo $OldBases = @('C:\xampp','D:\xampp')
...
>> "install\_xampp_path_fix.ps1" echo   $DefaultSameDrive = (($RootUnix.Substring(0,2)) + '/xampp')
>> "install\_xampp_path_fix.ps1" echo   if ($RootUnix.StartsWith($DefaultSameDrive, [System.StringComparison]::OrdinalIgnoreCase)) { $extra = $RootUnix.Substring($DefaultSameDrive.Length); if ($extra) { $Out = $Out.Replace($RootUnix + $extra, $RootUnix); $Out = $Out.Replace(($Root + ($extra -replace '/','\')), $Root) } }
